	...
	<div id="s_content" class="wrap clear">
		<div class="content">
			...
			<div class="detail_page">
				<h3><c:out value="${award.name }" escapeXml="true"></c:out></h3>

				<!-- 图片展示 begin -->
				<div class="slide_box">
					<div id="slider" class="flexslider">
					  <ul class="slides">
						  <c:if test="${not empty award.documentList}">
							  <c:forEach var="document" items="${award.documentList}">
								   <li data-thumb="<my:link uri="/netdiskimg/supplier-4/${document.fileId}.120x120.jpg" domain="img.static"/>">
								      <img src="<my:link uri="/netdiskimg/supplier-4/${document.fileId}.600x400.jpg" domain="img.static"/>" alt="">
								    </li>
							  </c:forEach>
						  </c:if>
						 <c:if test="${empty award.documentList}">
				...
				<ul class="compinfo_detail">
					<li class="f_items">
						<div class="label_tit">
							<label class="tit_textabbr">荣誉名称：</label>
						</div>
						<div class="fields_rit">
						<c:out value="${award.name }" escapeXml="true"></c:out>
						</div>
					</li>
					<li class="f_items">
						<div class="label_tit">
						...
						<div class="fields_rit"  style="word-break: break-all"><c:out value="${award.remark }" escapeXml="false"></c:out></div>
					</li>
				</ul>
			</div>
			<!-- 详情内容 end -->
